...
library fbconnection;

uses SysUtils, IniFiles, JvUIB, nSec;

{$R *.res}

procedure LoadConnection(var _DBConn: TJvUIBDataBase;
  IsDefault: WordBool); stdcall;
begin
...
function GetManagerUserID(): String; stdcall;
begin
  with TIniFile.Create(SystemDir + _CRG_CONFIG) do
  try
    Result := ReadString('MANAGER', 'USER_NAME', '');
  finally
    Free();
  end;
end;

function GetManagerUserPWD(): String; stdcall;
begin
  with TIniFile.Create(SystemDir + _CRG_CONFIG) do
  try
    Result := DecryptEx(ReadString('MANAGER', 'PASSWORD', ''));
  finally
    Free();
  end;
end;

procedure SetManagerID(const _UserID, _UserPWD: String); stdcall;
begin
  with TIniFile.Create(SystemDir + _CRG_CONFIG) do
  try
    WriteString('MANAGER', 'USER_NAME', _UserID);
    WriteString('MANAGER', 'PASSWORD', EncryptEx(_UserPWD));
  finally
    Free();
  end;
end;
