          ...
          begin
            ...
            if cdsTemp.recordcount>0 then
              begin
                ...
                edAccNo.Text := cdsTemp.fieldbyname('BookNo').AsString;
...
var frm:TfrmSelectBank;
    UseAccBnkCode : boolean;
begin
  UseAccBnkCode := (TString(cmbPayType.Items.Objects[cmbPayType.ItemIndex]).Str<>'28');

  with cdsBankCode do
  begin
    close;
    if not UseAccBnkCode then
      CommandText:='select bk_code as bankid,bk_code as bankcode,bk_title as bankname from bank_code where bk_code ='''+trim(edBank.Text)+''''
    else
    ...
    begin
     ...
     // edBankName.Text:=fieldbyname('bk_title').AsString;
      ...
      close;
        if not UseAccBnkCode then
          CommandText:='select bk_code as bankid,bk_code as bankcode,bk_title as bankname from bank_code where bk_code like '''+trim(edBank.Text)+'%'''
        else
