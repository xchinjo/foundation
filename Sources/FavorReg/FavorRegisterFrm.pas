...
//  try
    ...
    if TxtSp.Text ='' then
      begin
        Application.MessageBox('กรุณากรอกข้อมูลให้ครบ', 'ข้อมูลไม่ครบ', MB_OK + MB_ICONQUESTION);
        Exit ;
      end;
    if Total < CdsSpCode.fieldByName('sp_amount').AsFloat then
      begin
        Application.MessageBox('ยอดเงินที่เลือกไม่พอ กรุณาเลือกบิลเพิ่ม','ยอดเงินที่เลือกไม่พอ', MB_OK + MB_ICONWARNING);
        Exit ;
      end;
    ...
    begin
      Application.MessageBox(pchar('ไม่สามารถทำการบันทึกได้ กรุณาตรวจสอบความถูกต้อง'),pchar('Warning'),MB_OK or MB_ICONWARNING);
      exit;
    end;
    ...
    Application.MessageBox('ทำการบันทึกบัตรอุปการะคุณเรียบร้อย', 'บันทึก',MB_OK) ;
    CdsSP.Edit ;
//  except
//  end ;
  x := 'BillWork' ;
...
begin
  if Str = '1' then
    begin
      if Application.MessageBox('บัตรอุปการะคุณ ที่เคยขอคือขั้นสูงสุดแล้ว ต้องการทำต่อ',
        'บัตรอุปการะคุณ', MB_OKCANCEL) = IDCANCEL then
      begin
        exit ;
      end;
    end
