...
begin
  try
    ...
    if cdsBillDetails.Active then
      if cdsBillDetails.RecordCount<=0 then
      begin
        MessageDlg('ไม่สามรถทำการบันทึกได้ เนื่องจากยังไม่มีรายการ',mtWarning,[mbOK],0);
        exit;
      end;

    if currPay< currAmount then
    begin
        MessageDlg('ยังทำรายการบริจาคไม่ครบตามจำนวน ไม่สามารถบันทึกได้',mtWarning,[mbOK],0);
        exit;
    end;
        ...
        if cdsBillDetails.Active then
          ...
          begin
            ...
            begin
              ...
              MessageDlg('ทำการบันทึกเรียบร้อย',mtInformation,[mbOK],0);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MessageDlg('คุณต้องการขอเครื่องราช ชั้นที่ '+inttostr(currRoyalLevel-1)+' หรือไม่',mtConfirmation,[mbYes,mbNo],0);
      end;
