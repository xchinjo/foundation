...
begin
  ...
  if TxtRoyal.Text ='' then
    begin
      Application.MessageBox('กรุณากรอกข้อมูลให้ครบ', 'ข้อมูลไม่ครบ', MB_OK or MB_ICONINFORMATION);
      Exit ;   // no royal Code
    end;

  if Total < CdsRYCode.fieldByName('ry_amount').AsFloat then
    begin
      Application.MessageBox('ยอดเงินที่เลือกไม่พอ กรุณาเลือกบิลเพิ่ม','ยอดเงินที่เลือกไม่พอ', MB_OK or MB_ICONINFORMATION);
      x := 'BillWork' ;
   ...
   if (Cur_Do = '') or (Cur_Do = null) then
     begin
      if (TxtRoyal.KeyValue >= Do_Level) and (Do_Level <> '') then
        begin
          Application.MessageBox('ระดับขั้นที่เลือกต่ำกว่าระดับขั้นเดิม กรุณาเลือกระดับขั้นที่สูงกว่า','ระดับขั้น', MB_OK + MB_ICONWARNING);
          exit ;
        end;
     end
   else
    ...
    if Do_End = 'N' then
      begin
        if Application.MessageBox('ขณะนี้ยังอยู่ในระหว่างการขอเครื่องราช ต้องการทำต่อ','ันทึก', MB_OKCANCEL + MB_ICONQUESTION) = IDCANCEL then
          begin
            exit ;
          end;
      end;
...
//  ShowMessage(FloatToStr(CdsRoyal.FieldByName('rytotal').AsFloat )) ;
  ...
  Application.MessageBox('ทำการบันทึกการของเครื่องราชเรียบร้อยแล้ว','บันทึก', MB_OK);
